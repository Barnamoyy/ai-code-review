...
export async function deletePreviousAIComments(repo, prNumber) {
  ...
  sendLog({
    type: "info", 
    message: "Deleting previous AI comments",
    source: "github_api",
    repo,
    prNumber,
  })
  ...
  try {
    ...
    for (const comment of aiComments) {
      try {
        ...
        sendLog({
          type: "warn", 
          message: `Could not delete comment #${comment.id}`, 
          source: "github_api",
          repo,
          prNumber,
          error: err.message,
    ...
    sendLog({
      type: "info", 
      message: `Deleted ${aiComments.length} AI comments`,
      source: "github_api",
      repo,
      prNumber,
    ...
    sendLog({
      type: "error", 
      message: "Error deleting AI comments",
      source: "github_api",
      repo,
      prNumber,
      error: err.message,
...
export async function handlePullRequestEvent(payload) {
  ...
  if (action === "opened" || action === "synchronize") {
    try {
      ...
      sendLog({
        type: "error", 
        message: "Error handling pull request event", 
        source: "github_webhook",
        repo,
        prNumber,
        error: error.message,
...
export async function postReviewCommentsBatch(
  ...
  sendLog({
    type: "info", 
    message: "Starting batch review post", 
    source: "github_api",
    repo,
    prNumber,
  ...
  if (githubComments.length === 0) {
    sendLog({
      type: "warn", 
      message: "No valid comments to post", 
      source: "github_api",
      repo,
      prNumber,
    })
  ...
  const CHUNK_SIZE = 50;
  const chunks = [];
  for (let i = 0; i < githubComments.length; i += CHUNK_SIZE)
    chunks.push(githubComments.slice(i, i + CHUNK_SIZE));
  ...
  try {
    for (const chunk of chunks) {
      ...
      sendLog({
        type: "info", 
        message: `Posted ${chunk.length} AI review comments`,
        source: "github_api"
      })
    }
    ...
    try {
      ...
      sendLog({
        type: "error", 
        message: "Error updating database after review", 
        source: "github_api",
        repo,
        prNumber,
        error: dbErr.message,
      ...
      console.error(
        "Failed to update database after successful review:",
        dbErr.message
      );
    }
  } catch (err) {
    sendLog({
      type: "error", 
      message: "Error posting batch review", 
      source: "github_api",
      repo,
      prNumber,
